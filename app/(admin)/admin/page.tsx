...
const STATUS_META: Record<string, { label: string; color: string }> = {
  PROGRAMADA: { label: "Programadas", color: "#2a78d6" },
  EN_CURSO: { label: "En curso", color: "#eda100" },
  FINALIZADA: { label: "Finalizadas", color: "#1baf7a" },
  CANCELADA: { label: "Canceladas", color: "#e34948" },
};
        ...
        <>
          ...
          <div className="hovercard relative rounded-2xl border border-teal-100 bg-white p-6 dark:border-teal-900/40 dark:bg-slate-900">
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              <Link href="/admin/citas" className="text-sm font-semibold text-teal-600 hover:underline">
                Ver todas
              </Link>
            </div>
            {upcoming.length === 0 ? (
              ...
              <ul className="divide-y divide-slate-100 dark:divide-slate-800">
                {upcoming.map((a, idx) => (
                  <li key={`appt-${a.idCita}-${idx}`} className="flex items-center gap-3 py-3 text-sm">
                    <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: STATUS_META[a.estado]?.color ?? "#94a3b8" }} />
                    <span className="font-medium text-slate-900 dark:text-white">{a._petName ?? "—"}</span>
                    <span className="text-slate-500">
                      con {a._vetName || "—"}
                    </span>
                    <span className="ml-auto tabular-nums text-slate-500">
                      {a.fechaProgramada?.slice(0, 16).replace("T", " ")}
                    </span>
                  </li>
                ))}
              </ul>
