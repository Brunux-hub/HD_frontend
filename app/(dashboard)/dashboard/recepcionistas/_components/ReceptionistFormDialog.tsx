...
type Props = {
  users: UserItem[];
  mode?: "create" | "edit";
  data?: ReceptionistItem;
  icon?: LucideIcon;
  buttonColor?: "default" | "success" | "alert";
  onSubmit: (data: CreateReceptionistRequest) => Promise<void>;
};

const ReceptionistFormDialog = ({
  users,
  mode,
  data,
  icon: Icon,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: CreateReceptionistRequest = {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "No se pudo guardar el recepcionista.",
      );
    } finally {
      setSubmitting(false);
    }
          ...
          <FieldGroup>
            <FieldSet>
              <FieldLegend className="text-center text-xl font-semibold">
                {mode === "create" ? "Crear Recepcionista" : "Modificar Recepcionista"}
              </FieldLegend>
              <FieldGroup>
                <Field>
